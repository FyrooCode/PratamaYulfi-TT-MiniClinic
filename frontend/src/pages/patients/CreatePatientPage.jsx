...
export default function CreatePatientPage() {
  ...
  // RBAC Guard: Doctor cannot register patients
        ...
        <Card className="border-amber-200 bg-amber-50/50">
          <CardHeader>
            ...
            <CardDescription className="text-amber-700">
              Hanya staf Resepsionis dan Admin yang memiliki izin untuk mendaftarkan pasien baru.
            </CardDescription>
          </CardHeader>
          <CardContent>
            <Button
    ...
    try {
      ...
      if (res.success && res.data) {
        ...
        showNotification({
          ...
          message: `Pasien "${res.data.name}" berhasil terdaftar dengan No. RM: ${res.data.medical_record_number}`,
        });
        navigate('/patients');
      }
    } catch (err) {
      ...
      if (err.response?.status === 409) {
        setErrors((prev) => ({ ...prev, nik: 'NIK ini sudah terdaftar di sistem' }));
        showNotification({
          type: 'error',
  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto pb-12">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-slate-500 mt-1 max-w-[65ch]">
              Lengkapi formulir di bawah ini untuk membuat data rekam medis pasien baru.
            </p>
          </div>
        </div>
      </div>

      {/* Auto-generated MRN Information Banner */}
      <div className="rounded-lg border border-teal-200 bg-teal-50/60 p-4 flex items-start gap-3">
        ...
        <div className="text-sm text-slate-700 space-y-1">
          <p className="font-semibold text-teal-900">
            Nomor Rekam Medis (No. RM) Otomatis
          </p>
          <p className="text-slate-600 leading-relaxed text-sm max-w-[65ch]">
            Nomor Rekam Medis pasien akan digenerate otomatis oleh sistem dengan format standar{' '}
            <span className="font-semibold text-teal-800">RM-YYYY-XXXX</span> saat formulir berhasil disimpan.
          </p>
        </div>
      </div>
      ...
      {/* Media Alert Dialog for Confirming Patient Registration */}
        ...
        <AlertDialogContent className="max-w-md bg-white rounded-xl shadow-xl border border-slate-200">
          <AlertDialogHeader className="items-center text-center sm:items-start sm:text-left">
            ...
            <AlertDialogDescription className="text-sm text-slate-600 leading-relaxed pt-1">
              Mohon pastikan seluruh data identitas pasien di bawah ini sudah sesuai sebelum disimpan ke sistem rekam medis.
            </AlertDialogDescription>
          </AlertDialogHeader>
